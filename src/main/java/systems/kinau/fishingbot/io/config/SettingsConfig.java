...
public class SettingsConfig implements Config {
    ...
    @Property(key = "server.ip", description = "config-server-ip") private String serverIP = "wolfxmc.org";
    ...
    @Property(key = "server.online-mode", description = "config-server-online-mode") private boolean onlineMode = false;
    ...
    @Property(key = "auto.auto-command-on-respawn.commands", description = "config-auto-auto-command-on-respawn-commands") private List<String> autoCommandOnRespawn = Arrays.asList("朕复活了", "/home fishing");
    ...
    @Property(key = "auto.auto-command-before-death.commands", description = "config-auto-auto-command-before-death-commands") private List<String> autoCommandBeforeDeath = Arrays.asList("今日我虽死，但还是西楚霸王！", "/home");
    ...
    @Property(key = "auto.timer.timers", description = "config-auto-timers") private List<Timer> timers = Collections.singletonList(new Timer("test", 5, TimeUnit.MINUTES, Collections.singletonList("欢迎/res tp commune来社区参观")));

    @Property(key = "account.mail", description = "config-account-mail") private String userName = "aholk";
    @Property(key = "account.password", description = "config-account-password") private String password = "CHANGEME";
    @Property(key = "account.auth-service", description = "config-account-auth-service") private AuthService authService = AuthService.WOLFXMC;
    ...
    @Property(key = "announces.announce-type-chat", description = "config-announces-announce-type-chat") private AnnounceType announceTypeChat = AnnounceType.NONE;
    ...
    @Property(key = "announces.announce-lvl-up-text", description = "config-announces-announce-lvl-up-text") private String announceLvlUpText = "老子%lvl%级了卧槽";

    @Property(key = "start-text.enabled", description = "config-start-text-enabled") private boolean startTextEnabled = true;
    @Property(key = "start-text.text", description = "config-start-text-text") private List<String> startText = Arrays.asList("独钓寒江雪，孤舟蓑笠翁", "/trigger Bot");
