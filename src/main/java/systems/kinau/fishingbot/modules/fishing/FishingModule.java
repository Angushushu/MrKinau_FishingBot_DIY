...
public class FishingModule extends Module implements Runnable, Listener {
    ...
    //
    boolean autoHeadshaking = FishingBot.getInstance().getCurrentBot().getConfig().isAutoHeadshakingEnabled();
    float angle = FishingBot.getInstance().getCurrentBot().getConfig().getAutoHeadshakingAngle(); //
    int frequency = FishingBot.getInstance().getCurrentBot().getConfig().getAutoHeadshakingFrequency();//
    ...
    public void fish() {
        ...
        new Thread(() -> {
            ...
            if(autoHeadshaking)
                FishingBot.getInstance().getCurrentBot().getPlayer().look(currentYaw + angle, 0, 5);//
